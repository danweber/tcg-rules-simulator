...
export class Instance {
    ...
    JSON_instance(): any {
        let instance = {
            ...
            stack: this.pile.map(x => `${x.id}@${x.s_colors()}`),
            sa: this.get_sa(),
            loc: this.location,
    ...
    check_preflight(sfx: SubEffect[]): SolidEffect[] {
        logger.info(`Checking preflight instance ${this.id} name ${this.name()} effects ${sfx.join(",")}`);
        let ret: SolidEffect[] = [];
    ...
    is_token(): boolean { logger.error("no tokens at all"); return false; }
    ...
    can_attack(td?: TargetDesc): (false | number[]) {
        // I'm manually parsing td
        if (this.game.n_turn == this.play_turn && !this.has_keyword("Rush")) {
            // How do I find out if I'm in an effect that lets me attack the turn I'm summoned
            // Maybe I pass in an AtomicEffect instead of a target.
            // Cheating for now. 
            if (!td || td.raw_text != "your opponent's monster") {
                return false;
            }
        }
