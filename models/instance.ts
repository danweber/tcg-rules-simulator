...
*/
    ...
    static one_effect_preflight(my_fx: SolidEffect, sfx: SubEffect[], me: TargetSource, n_me_player: number, game: Game, thus: Instance | CardLocation): boolean {

        let ret: SolidEffect[] = [];

        if (!my_fx.interrupt) return false;
            ...
            for (let my_interrupter of my_interrupters) {
                ...
                if (Instance.match_certain_effect(g_fx, my_interrupter, n_me_player)) {
                    ...
                    if (g_fx.game_event == GameEvent.MOVE_CARD) {
                        ...
                        logger.debug("SPECIAL INST MADE TO BE " + thus.id);
                        my_fx.source = me;
    ...
    check_preflight(sfx: SubEffect[]): SolidEffect[] {
        ...
        for (let my_fx of this.all_effects()) {
            if (Instance.one_effect_preflight(my_fx, sfx, new SpecialInstance(this), this.n_me_player, this.game, this))
                ret.push(my_fx);
        }
    ...
    static collect_persistent_effect(cc: SolidEffect, game: Game, n_me_player: number, source: TargetSource):
        ...
        if (w.td.conjunction == Conjunction.PLAYER || w.td.raw_text.match(/security/)) {
            ...
            // what does returning "undefined" mean? I think that it's just ignored.
            console.error(988, "try removing me");
            return [undefined];
        } else {
