...
export class Instance {
            ...
            for (let g_fx of sfx) {
                ...
                if (my_matching_rx = my_reactors.find(x => Instance.match_certain_effect(g_fx, x, this.n_me_player))) {
                    ...
                    if (ge == GameEvent.MOVE_CARD) {
                        match = this.match_move(g_fx, my_matching_rx);
                        ...
                        if (!Instance.compare_cause(g_fx, my_matching_rx)) {
                        ...
                        if (my_matching_rx.td.matches(this, me, this.game)) {
    ...
    check_preflight(sfx: SubEffect[]): SolidEffect[] {
        ...
        for (let my_fx of this.all_effects()) {
            ...
            let my_interrupters = my_fx.interrupt;
            let what_triggers_me = [];
            for (let g_fx of sfx) {
                ...
                for (let my_interrupter of my_interrupters) {
                    ...
                    if (Instance.match_certain_effect(g_fx, my_interrupter, this.n_me_player)) {
                        ...
                        if (!my_interrupter.td.matches(g_fx.chosen_target, me, this.game))
                            continue;
                        // more things should be made generic like this
                        if (g_fx.game_event == GameEvent.MOVE_CARD) {
                            if (this.match_move(g_fx, my_interrupter)) {
                                my_fx.source = new SpecialInstance(this);
                                what_triggers_me.push(g_fx);
                            }
                        } else if (g_fx.chosen_target == this && my_interrupter.td.matches(this, me, this.game)) {
    ...
    static match_certain_effect(actual_event: SubEffect, candidate: InterruptCondition, n_me_player: number): boolean {
        ...
        if (
            ...
            let cause = Instance.compare_cause(actual_event, candidate);
    ...
    static compare_cause(actual_event: SubEffect, candidate: InterruptCondition): boolean {
    ...
    match_move(se: SubEffect, ic: InterruptCondition): boolean {
    ...
    static collect_persistent_effect(cc: SolidEffect, game: Game, n_me_player: number, source: TargetSource):
        ...
        if (w.td.conjunction == Conjunction.PLAYER || w.td.raw_text.match(/security/)) {
            ...
            //XX.do_terminus_effect(1, w, undefined, game);
            return [undefined];
        } else {
    ...
    */
    ...
    can_evolve(card: Card, available_memory: number, fusion_evo_pos: number, type: 'evo'|'fusion'|'burst'): number[] | false {
        ...
        let ret: number[] = [];
        logger.info(`I am ${this.name()} level ${this.get_level()} with colors ${this.s_colors()}`);
        ...
        logger.info("can i evo into " + JSON.stringify(conditions));
        ...
        logger.debug("returning 1174 " + JSON.stringify(ret));
