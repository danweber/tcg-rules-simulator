...
export class Instance {
    ...
    constructor(game: Game, me_player: Player, other_player: Player) {
        this.kind = "Instance";
    ...
    all_active_statuses(): StatusCondition[] {
        ...
        for (let sc of status_conditions) { // this.expiring_status_effects) {
            logger.debug(`SEE IF CAN DIGI ${status_cond_to_string(sc)} `);
    ...
    // * may not do what you expect for "once per turn"; move towards JP rules announced around 21-Jun-2024
    ...
    check_posteffect(sfx: SubEffect[]): SolidEffect[] {
        ...
        // If I have an effect like "draw 1 for each digimon deleted",
            ...
            for (let g_fx of sfx) {
                ...
                if (my_matching_rx = my_reactors.find(x => x.ge == g_fx.game_event)) {
                    ...
                    if (ge == GameEvent.SUSPEND || ge == GameEvent.DELETE || ge == GameEvent.PLAY || ge == GameEvent.EVOLVE) {
                        ...
                        logger.warn("Just matching because event matches by default");
                        match = true;
                    }
                    if (match) {
        ...
        for (let my_fx of this.all_effects()) {
            ...
            for (let g_fx of sfx) {
                ...
                if (g_fx.game_event == my_interrupter.ge) {
                    logger.debug("events kinds match up: " + GameEvent[ge]);
                    ...
                    if (true && my_interrupter.ge == GameEvent.EVOLVE) {
    ...
    can_do(effect: SubEffect, force_apply_status: boolean = true): boolean {
        for (let sc of this.all_statuses()) { //  expiring_status_effects) {
            logger.debug(`sc.s is ${sc.s}  --> ${GameEvent[effect.game_event]}`);
            if (sc.s.immune) {
                // When looking at "immune to opponent's digimon effects" we need to
    ...
    get_playcost(): number { return this.top().p_cost; }
    ...
    //    All of your Digimon with [Xros Heart] in its traits or <Retaliation> gain <Rush> and <Blocker>.
    /// that one finds the targets and applies the effects

    // While [Rapidmon] or [X Antibody] is in this Digimon's evolution cards, all of your opponent's suspended Digimon get -4000 DP.
    //// use 'while X' to determine if I fire or not
    //// then target all suspended digimon and apply constant effect
    ...
    // All of your opponent's Digimon gain \"[When Attacking] Lose 2 memory.\" until the end of your opponent's next turn.",
    //// GAME EFFECT: all targets get this solideffect

    // While this Digimon is suspended, all of your Security Digimon get +5000 DP."
    //// no real targets

    // While this Digimon has <Piercing>, it gets +2000 DP.
    ...
    is_digimon(): boolean { return this.top().is_digimon() || this.top().is_egg(); }
    ...
    can_attack(): (false | number[]) {
        if (this.game.n_turn == this.play_turn) return false;
        if (!this.has_dp()) return false;
        if (!this.in_play()) return false;
        if (!this.is_ready()) return false; // technically not needed with below suspend check
        ...
        for (let mon of this.other_player.field) {
            // TODO: make this.can_do() handle the is_ready and is_digimon clauses automatically
            if (!mon.is_ready() && mon.is_digimon() && this.can_do(declare)) {
                ret.push(mon.id);
            }
        }
    ...
    can_evolve(card: Card, available_memory: number): number[] | false {
        ...
        for (let sc of status_conditions) { // this.expiring_status_effects) {
            logger.debug(`SEE IF CAN DIGI ${status_cond_to_string(sc)} `);
            if (this.can_do(sc.parent_subeffect!, false)) {
                if (sc.s.game_event == GameEvent.MAY_DIGI_FROM) {
                    logger.info(`we can digi into ${sc.s.n_mod}`);
        ...
        for (let digicond of conditions) {
            if (digicond.text) console.warn(digicond);
            logger.debug(`looking up digi condition ${Color[digicond.color!]} level ${digicond.level} cost ${digicond.cost}`);
            // we have to match all things that exist
            if (digicond.color && !this.has_color(digicond.color)) continue;
            if (digicond.level && digicond.level != this.get_level()) continue;
            if (digicond.name_is && !this.name_is(digicond.name_is)) continue;
            if (digicond.name_contains && !this.name_contains(digicond.name_contains)) continue;
            if (digicond.trait && !this.has_trait(digicond.trait)) continue;
            ret.push(digicond.cost);
        }
    ...
    do_evolve(_card: Card) {  // doesn't check if we "can"
        ...
        if (this.location != Location.FIELD &&
            ...
            logger.error('digivolving in wrong place ' + this.location + " " + this.id + " " + this.get_name());
            let a: any = null; a.digi_location();
        }
        // how do I remove the card from hand??        
        let card = _card.extract();
        card.move_to(this.location, this);
        this.history(`Digivolved to ${card.name} ${card.id} for cost ${card.e_cost} `);
        logger.info("Digivolved to " + card.id + " on turn " + this.game.n_turn);
        this.game!.ui_card_move();
        // trigger card's on digivovling and other "when evolve" fx
    }
    ...
    deevolve(count: number, args: any = {}): boolean {
        ...
        while (count > 0) {
            ...
            if (this.has_level() && this.get_level() == 3 && !args['force']) {
                this.log("Monster is level 3, cannot de-digi any more");
