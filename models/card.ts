...
export class Card {
    ...
    _parse_special_evolve(line: string, evo: any, no_push: boolean = false): any {
        ...
        if (m = line.match(/^(red|blue|yellow|green|black|purple|white) Lv.(\d)\s+: Cost (\d+)$/i)) {
            ...
            // Lv.6 w/[xxx]\u00a0in its name w/o [yyy]\u00a0trait: Cost 1
        } else if (m = line.match(/^\s*(Lv.(\d))? ?w\/\[?(\w*)\]?.in.its.name(\s*w.o \[(.*?)\].trait)?: Cost (\d)$/)) {
            //console.error(m);
            ...
            evo.two_color = true;
        } else if (m = line.match(/^\s*(Lv.(\d))? ?w\/\[([^\]]*)\].trait: Cost (\d)$/)) {
            if (m[2]) evo.level = parseInt(m[2]);
            evo.trait = m[3]
            evo.cost = parseInt(m[4]);
        ...
        } else if (m = line.match(/^\s*(Lv.(\d))? ?w\/\[([^\]]*)\].in.its.name (?:&|and) w\/\[(.*)\].trait: Cost (\d)$/)) {
            if (m[2]) evo.level = parseInt(m[2]);
            evo.trait = m[4]
            evo.name_contains = m[3];
    ...
    constructor(blob: any, text: string = "") {
        ...
        if (app_format) {
            ...
            if (fusion != "-") {
                // this is duplicated
                if (fusion.match(/.DNA.Evolve./)) {
                    fusion = fusion.substring("[DNA Evolve]".length).trim();
                    this.parse_fusion_evolve(fusion, {});
                }
            }
    ...
    public can_merge_evolve(left: Instance | CardLocation, right: Instance | CardLocation): false | number[] {
        // check if the left can evo
        logger.debug(`can merge? ${this.name} onto ${left.get_name()} and ${right.get_name()}`);
        let l = left.can_evolve(this, 99, 1);
        logger.debug(`l is ${l} ${!!l}`);
        if (!l) return false;

        let r = right.can_evolve(this, 99, 2);
