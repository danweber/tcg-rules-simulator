...
import { GameTest, SpecialCard, TargetDesc, TargetSource } from './target';
...
export class Card {
    ...
    parse_link_requirement(line: string, evo: any) {
        ...
        if (m) {
            ...
            this.link_requirements.push(link);
        }
    }
    ...
    _parse_special_evolve(line: string, evo: any, no_push: boolean = false): any {
        ...
        //     console.error(line);
        let m;
        // red level 4: only mostly for fusions
    ...
    return_card() {
        ...
        console.error(1041, old_cl && Location[old_cl.location]);
        this.extract();
        if (old_cl) {
    ...
    public text_contains(str: string): boolean {
        str = str.toLowerCase();
        console.error(1270, this.main_text);
        console.error(1271, this.evo_text);
        return this.name.toLowerCase().includes(str) ||
            this.name_rule?.toLowerCase().includes(str) ||
            this.trait_rule?.toLowerCase().includes(str) ||
            this.main_text.toLowerCase().includes(str) ||
            this.inherited_text.toLowerCase().includes(str) ||
            this.security_text?.toLowerCase().includes(str) ||
            this.evo_text?.toLowerCase().includes(str) ||
            this.fandom_input.toLowerCase().includes(str)
    }
...
export class CardLocation {
    ...
    extract(): Card {
        ...
        let cl = new CardLocation(this.game!, this.card.n_player, this.location, i, this.instance);
