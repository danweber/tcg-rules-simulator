...
enum CombatStep {
    ...
    // ...
    // ...
    // ... I just have this 4-tuple of 
    // {pre-flight events, interrupters, do events, responders}
    // all over the place. It should probably be a common loop.

    /*    PREFLIGHT_BATTLE_RESULTS,
        GET_INTERRUPTERS,
        DO_EVENTS,
        GET_RESPONDERS,*/
    ...
    FINISH_SEC_BATTLE, // move card to trash, back up to CONTINUE_TO_SECURITY_STACK
...
export class CombatLoop {
    ...
    constructor(g: Game, source: number, target: number, p: Player) {
        ...
        //console.log(this.defender_i && this.defender_i.name());
    ...
    step(): false | SubEffect[] {
        if (this.s == CombatStep.COMBAT_SETUP) {
            console.error("combat set up failed?");
            return [];
        }
        ...
        if (this.s == CombatStep.GET_COUNTER_RESPONSES1) {
            ...
            console.log("can blast is " + can_blast);
            ...
            this.game.wait(this.defender_p.player_num, blast);
        ...
        if (this.s == CombatStep.PROCESS_COUNTER_RESPONSES) {
            ...
            let ans = parseInt(this.game.get_answer());

            console.log("blast answer is " + ans);
            if (ans == -1) {
                this.s = CombatStep.ASK_BLOCKER_RESPONSES;
                return false;
            }

            let n_inst = Math.floor(ans / 50);
            let n_card = ans % 50;
            console.log(n_inst, n_card);
            let c: Card = this.defender_p.hand[n_card];
            let inst: Instance = this.game.get_instance(n_inst);
            console.log("===");
            console.log("card " + c.id + " " + inst.name);
            console.log("inst " + inst.id + " " + inst.name);
            /*
            console.log(this.defender_p.can_evolve(c, false));

            console.log(this.defender_p.can_evolve(c, true));
            console.log("===");*/
            if (!c || !inst ||
                !this.defender_p.can_evolve(c, true) ||
                !inst.can_evolve(c, 100)) {
                this.game.la("Blast digi targets invalid");
                console.error("CANNOT DIGI BLAST");
                this.s = CombatStep.GET_COUNTER_RESPONSES1;
                return false;
            }
            this.game.la(`PLAYER ${this.defender_p.player_num} BLAST DIGIVOLVES ${c.name.toUpperCase()} ONTO ${inst.get_name().toUpperCase()}`);
            // this needs to be a interrupable respondable event
            let td = new TargetDesc("");
            let e: SubEffect[] = [];
            e.push({
                cause: EventCause.EFFECT, // blast digi is an effect
            ...
            this.directed_subeffect_loop = new DirectedSubEffectLoop(this.game, e);
        ...
        if (this.s == CombatStep.GET_BLOCKER_RESPONSES1) {
            ...
            this.directed_subeffect_loop = new DirectedSubEffectLoop(this.game, [block_suspend, target_switch]);
            console.log("BLOCKING WITH " + this.defender_i.get_name());
            this.game.log("Attack target changed to " + this.defender_i.get_name());
...
//            console.log(this.defender_i.name);
        ...
        if (this.s == CombatStep.CHECK_FIELD_COMBAT) {
            ...
            if (!this.attacker.in_play() || !this.attacker.has_dp() ||
                !this.defender_i.in_play() || !this.defender_i.has_dp()) {
                this.game.announce("Battle can no longer happen: combatant removed");
            ...
            let m = `attacker is ${this.attacker.name()} ${this.attacker.dp()} ` +
                ` ${retaliate_a ? "with retaliation" : ""} defender is ` +
                `${this.defender_i.get_name()} ${this.defender_i.dp()} ${retaliate_d ? " with retaliate" : ""} `;
            this.game.la(m);
            ...
            console.log("SETTING SPEC SOURCE");
            console.log(this.del_attacker);
            console.log(del_defender);


            if (this.attacker.dp() == this.defender_i.dp()) {
            ...
            } else if (this.attacker.dp() <= this.defender_i.dp()) {
                this.combat_deletion_effects.push(this.del_attacker);
                this.game.la("Attacker loses in battle");
            } else if (this.defender_i.dp() <= this.attacker.dp()) {
                this.combat_deletion_effects.push(del_defender);
                this.game.la("Defender loses in battle");
            } else {
                console.error("wut");
            }
            console.log("we have  " + this.combat_deletion_effects.length + " deletions to try");
            this.s = CombatStep.PREFLIGHT_COMBAT_RESULTS;

            // constructoir
            this.directed_subeffect_loop = new DirectedSubEffectLoop(this.game, this.combat_deletion_effects);
        ...
        if (this.s == CombatStep.FINISH_FIELD_COMBAT) {
            ...
            if (piercing && !this.defender_i.in_play()) {
                ...
                // step. The digimon keeps it for battle even if they lose the
        ...
        if (this.s == CombatStep.CHECK_FOR_VICTORY) {
            ...
            if (this.defender.kind == "Instance" || this.defender instanceof Instance) {
                console.error("checking instance's security stack, error");
                console.trace();
                return true;
            }*/
        ...
        if (this.s == CombatStep.CONTINUE_TO_SECURITY_STACK) {
            ...
            console.log("defender has " + this.defender_p.expiring_status_effects.length + " effects");
            if (sec_card.has_dp()) {
                let delta_dp = 0;
                for (let stat of this.defender_p.expiring_status_effects) {
                    console.log("STAT IS " + status_cond_to_string(stat));
                    if (stat.s.game_event == GameEvent.DP_CHANGE) {
                        console.log("have dp change");
                        if (stat.parent_subeffect) {
                            console.log("have stat parent sub effect");
                            console.log("conjunction is " + Conjunction[stat.parent_subeffect.td.conjunction]);
                            console.log("Text is " + stat.parent_subeffect.td.text);
                            if (stat.parent_subeffect.td.text?.match(/security/i)) {
                                delta_dp += stat.s.n!;
                            }
                        }
                    }
            ...
            // hey, maybe I should move the [security] text to security. that would've been easier, huh?
            if (sec_card.security_text) {
                ...
                if (false) {
                    ...
                    console.log("fx of security " + fx.toString());
                ...
                this.combat_resolution_loop = new ResolutionLoop(this.game, [fx]);
        ...
        if (this.s == CombatStep.SECURITY_EFFECT_LOOP) {
            ...
            if (x) {
                console.log("done with security effect, go into security battle");
                this.s = CombatStep.SECURITY_CARD_REMOVED_LOOP_FIRST;
                return false;
            }
            return false;
        ...
        if (this.s == CombatStep.SECURITY_CARD_REMOVED_LOOP_FIRST) {
            ...
            let card_removed: SubEffect = {
                ...
                game_event: GameEvent.CARD_REMOVE_FROM_HEALTH,
                chosen_target: this.defender_p,
                td: new TargetDesc("dummy"),
                n_player: 3 - this.game.turn_player
            };

            this.directed_subeffect_loop = new DirectedSubEffectLoop(this.game, [card_removed]);
        ...
        if (this.s == CombatStep.SECURITY_CARD_REMOVED_LOOP_SECOND) {
            ...
            if (!x) return false;
            console.log("done with whatever responded to card being removed");
        ...
        if (this.s == CombatStep.SECURITY_BATTLE1) {
            ...
            if (more_deaths.length > 0) {
                this.directed_subeffect_loop = new DirectedSubEffectLoop(this.game, more_deaths);
            ...
            // trigger on deletion effects of attacker
            // resolve them
            // trigger end-of-battle effects like "play this at end of battle"
            // finally, move security card to trash//// could it already be there?


            //                this.log("done with this security attack.");
            //              this.log(`attack is in ${Location[attacker.location]}, sa is ${attacker.get_sa()} `);

            this.s = CombatStep.FINISH_SEC_BATTLE;
            // fall through
        }
        if (this.s == CombatStep.FINISH_SEC_BATTLE) {
        ...
        if (this.s == CombatStep.END_OF_ATTACK_EFFECT_LOOP_1) {
            ...
            console.log("solid end-of-attack count is " + (solids && solids.length));
            if (solids) {
