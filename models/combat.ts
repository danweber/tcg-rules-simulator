    ...
    step(): false | SubEffect[] {
        ...
        if (this.s == CombatStep.PROCESS_COUNTER_RESPONSES) {
            ...
            if (!c || !inst ||
                ...
                this.game.la("Blast evo targets invalid");
                console.error("CANNOT BLAST EVO");
                this.s = CombatStep.GET_COUNTER_RESPONSES1;
                return false;
            }
            this.game.la(`PLAYER ${this.defender_p.player_num} BLAST EVOLVES ${c.name.toUpperCase()} ONTO ${inst.get_name().toUpperCase()}`);
...
//            console.log(this.defender_i.name);
        ...
        if (this.s == CombatStep.CONTINUE_TO_SECURITY_STACK) {
            ...
            console.log("defender has " + this.defender_p.expiring_status_effects.length + " expiring and " +
                        this.defender_p.constant_status_effects.length + " constant effects");
            if (sec_card.has_dp()) {
                let delta_dp = 0;

                for (let stat of (this.defender_p.expiring_status_effects.concat(this.defender_p.constant_status_effects))) {
