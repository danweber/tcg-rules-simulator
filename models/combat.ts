...
export class CombatLoop {
    ...
    step(): false | SubEffect[] {
        ...
        if (this.s == CombatStep.PROCESS_COUNTER_RESPONSES) {
            ...
            logger.debug("blast answer is " + ans);
            if (ans == -1) {
                this.s = CombatStep.ASK_BLOCKER_RESPONSES;
                return false;
            }
            ...
            let n_card = ans % 50;
            logger.debug(`${n_inst}, ${n_card}`);;
            let c: Card = this.defender_p.hand[n_card];
            let inst: Instance = this.game.get_instance(n_inst);
            logger.debug("===");
            logger.debug("card " + c.id + " " + inst.name);
            logger.debug("inst " + inst.id + " " + inst.name);
            /*
            logger.debug(this.defender_p.can_evolve(c, false));

            logger.debug(this.defender_p.can_evolve(c, true));
            logger.debug("===");*/
            if (!c || !inst ||
                !this.defender_p.can_evolve(c, true) ||
                !inst.can_evolve(c, 100)) {
                this.game.la("Blast evo targets invalid");
                logger.error("CANNOT BLAST EVO");
                this.s = CombatStep.GET_COUNTER_RESPONSES1;
                return false;
            }
