...
import { EventCause, GameEvent, gerund } from './event';
...
import { for_each_count_target } from './util';
...
function can_pay(eff: AtomicEffect, game: Game, source: TargetSource): boolean {
    ...
    for (let i = 0; i < 1 /*eff.events.length*/; i++) {
        ...
        if (true || w.td.raw_text == "self" || w.td.raw_text == "this Monster") {
            ...
            let tgts = game.find_target(w.td, w.game_event, source, false, Location.SECURITY);
            ...
            if (legit_target_count < needed_targets) {
...
export class SolidEffectLoop {
    ...
    evolve_choices?: false | Array<[CardLocation, CardLocation | Instance, CardLocation | Instance | undefined, number?]>;
    ...
    step(): (false | SubEffect[]) {
        ...
        if (this.s == FakeStep.ASK_ACTIVATE_ACTUALLY) {
           ...
           */
            ...
            if (atomic_ask_to_activate(atomic_effect)) {
                ...
                let answers = [{ command: "1", text: `Activate ${text}${verb}`, ver: uuidv4() },
        ...
        if (this.s == FakeStep.ASK_TARGET1) {
            ...
            if (m = w.n_mod?.match(/counter,(td2|test),(\d+),([-0-9]+)/)) {
            ...
            if (w.td && w.td.conjunction == Conjunction.SOURCE) {
                ...
                if (w.game_event == GameEvent.EVOLVE) {
                    ...
                    let available_evos = player.get_all_evolves(false, fusion, this, this.effect.source, w.td2, w.td,
                        w.td3, w.n_mod);
                    ...
                    */
                    ...
                    if (answers.length == 1) {
                        ...
                        if (available_evos) {
                            let [cl, left, right, cost] = available_evos[0]!;
        ...
        if (this.s == FakeStep.DO_EFFECT_GO) {
            ...
            logger.debug("n mod game event" + GameEvent[this.effect.effects[this.n_effect].weirdo.game_event]);
            ...
            if (this.attack_target) {
                ...
                this.interrupter_loop = new InterrupterLoop(this.game, e, this.depth, !!eff.is_cost); // +1 or no?
            ...
            this.interrupter_loop = new InterrupterLoop(this.game, subfx, this.depth, !!atomic.is_cost); // +1 or no?
            this.events_to_do = []; // old array hangs around for loop, this is a new one
            this.s = FakeStep.DO_EFFECT_LOOP;
...
export class XX {
    ...
    // TODO: verify this 
    static do_terminus_effect(depth: number, weirdo: SubEffect, target: any, game: Game): boolean {
        ...
        */
        ...
        if (weirdo.game_event == GameEvent.DELETE) {
            ...
            let cl: CardLocation = weirdo.spec_source as CardLocation;
            let ts = new SpecialCard(cl);
            player.set_pending_effect(weirdo.delayed_effect!, weirdo.delayed_trigger!, game, ts);
            ...
            if (weirdo.status_condition) {
                ...
                for (let s of weirdo.status_condition) {
                    if (s.exp_description) {
                        ...
                        game.log(`Giving ${GameEvent[weirdo.status_condition[0].s.game_event]} to ${name}`);
            ...
            if (weirdo.chosen_target3) {
                ...
                for (let item of [instance, instance2]) {
                    if ("me_player" in item) { // instance
                        while (top = item.pile[0])
                            top.extract().move_to(Location.FIELD, fusioned, "BOTTOM");
            ...
            if ("cardloc" in target) {
                ...
                logger.debug("playing from card");
            ...
            if (!game.can_pay_memory(cost)) {
                ...
                game.la("Cannot pay for summon! Card goes back to hand.");
                c.extract();
            ...
            let cl = new CardLocation(game, p, Location.OPTZONE, 0);
            ...
            if (!game.can_pay_memory(cost)) {
                logger.warn("couldn't pay cost of " + cost);
                game.la("Cannot pay for use! Card goes back to hand.");
                return false;
            } else {
                game.la(`Use ${card.name} ${cost_msg}`);
                player.set_pending_effect(fx, Phase.ASAP, game, fx.source);
                game.pay_memory(cost);
                return true;
            }
...
export class InterrupterLoop {
    ...
    constructor(game: Game, effects: SubEffect[], depth: number, is_cost: boolean) {
        this.collected_events = [];
    ...
    step(): false | SubEffect[] {
        ...
        if (this.s == InterruptStep.DO_EFFECT_AT_LAST) {
            ...
            for (let se of this.effects_to_process!) {
                ...
                let paid = XX.do_terminus_effect(this.depth, se, target, this.game);
