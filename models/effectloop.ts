...
import { Card, CardLocation, DigivolveCondition } from './card';
...
export class SolidEffectLoop {
    ...
    evolve_choices: any; // DigivolveCondition[];
    //  evolve_choice?: DigivolveCondition;
        ...
        if (this.s == FakeStep.ASK_TARGET1) {
            ...
            if (true) {
                if (this.reacted_to.length > 0) {
                    ...
                    for (let subby of this.reacted_to) {
                        ...
                        //   solideffect       armor purge      decoy
                        //   interrupts        delete of self   delete of X && !self
                        //   cost              dedigi           delete self
                        //   cancels           the deletion     the deletion
                    }
                }
            }
            ...
            if (w.td && w.td.conjunction == Conjunction.SOURCE) {
                ...
                if (w.game_event == GameEvent.EVOLVE) {
                    // get targets tfor DIGIVOLVE BY EFFECT
                    ...
                    // I iterate over my cards in hand and see what they can digi into
                    // This is kind of backwards for the <Alliance> digi bonus.

                    let answers = [];
//                    let cards: CardLocation[] = player.hand;
                    console.log("yyyyyyy777777");
                    console.log(w.td.toString());
                    console.log(w.td2!.toString());
 ...
 //                   let cards = this.game.find_all(
                     ...
                     //  logger.info(`length is ${i} of ${cards.length}`);
                        ...
                        if (_digi)
                            ...
                            for (let [target, cost] of _digi) {
                                ...
                                let v = `Digivolve ${str} onto ${player.get_instance(target).get_name()} (${cost})`;
                                answers.push({
                ...
                if (w.game_event == GameEvent.MUST_ATTACK) {
                    ...
                    let attack_targets = mon.can_attack()
            ...
            if (!n_player || n_player < 1 || n_player > 3) { // do I mean 3?
                console.error("can't figure out target player: " + n_player);
                n_player = 0;
            }
        ...
        if (this.s == FakeStep.GET_TARGET) {
            ...
            if (this.evolve_choices) {

                let mon: Instance = this.effect.source.get_instance();
                let p = mon ? mon.n_me_player : -1;

                // assume DIGIVOLVE is always a solo effect -- which is a lie
...
//                weirdo.chosen_target = cl;    
        ...
        if (this.s == FakeStep.DO_EFFECT_LOOP) {
            ...
            // at that list??
            logger.info(this.rand + "going into stuff");
            ...
            if (!x.find(x => x.game_event == this.effect.effects[0].weirdo.game_event)) {
                //this.game.la("Cost not paid, attempt to cancel fails");
                successfully_paid = false;
            }
            logger.info(this.rand + "between pay clauses");
...
export class XX {
    ...
    static do_terminus_effect(weirdo: SubEffect, target: any, game: Game): boolean {
        ...
        let o_p = 3 - p;
        if (!p) {
            let aa: any = null; aa.rer();
        }
        logger.info("Terminus effect");
        console.dir(weirdo, { depth: 0 });
        logger.info("Target td is " + (weirdo.td ? weirdo.td.toString() : "nul"));
        ...
        */
        ...
        if (weirdo.game_event == GameEvent.DELETE) {
            ...
            if (weirdo.status_condition) {
                ...
                if (target) {
                    ...
                    if (weirdo.td.conjunction == Conjunction.PLAYER) {
                        ...
                        if (weirdo.td.text?.match(/their security/i)) {
        ...
        } else if (weirdo.game_event == GameEvent.DEVOVLVE) {
            game.log("Dedigivolving " + name + " by " + weirdo.n);
            ...
            if (true) {
                ...
                // obsolete when can-digi-from-trash or stack
            ...
            // .DIGIVOLVE has interrupted reduction
            ...
            let msg = `Digivolve instance ${instance.id} on ${Location[instance.location]} from ${original_name}` +
                ` for cost ${newcost} `;
            if (delta) {
            ...
            if ("cardloc" in target) {
                ...
                console.error("legacy play cost");
        ...
        } else if (weirdo.game_event == GameEvent.DIGISOURCE_ADD) {
            //            let p = weirdo.n_player!;
            let player = game.get_n_player(p!);
            //          let o_player = game.get_n_player(3 - p);
            ...
            console.error("assuming card add to health is always from deck");
            let player = game.get_n_player(p);
            ...
            if (weirdo.td.raw_text == "opponent") {
                // I'd like to use w.chosen_target to figure the target player
                let oppo = 3 - p!;
                let player = game.get_n_player(oppo);
...
export class DirectedSubEffectLoop {
    ...
    step(): false | SubEffect[] {
        ...
        if (!this.resolution_loop) {
            ...
            let res_loop = get_responder_loop(this.game, x);
            if (!res_loop) {
