...
import { AtomicEffect, InterruptCondition, Solid_is_triggered, StatusCondition, SubEffect, ic_to_plain_text, ic_to_string, status_cond_to_string } from "./effect";
...
function parse_if(line: string): GameTest {
    ...
    if (m = line.match(/you have (an? )?(\d or more)?(.*?)( in play)?$/i)) {
        return new GameTest(GameTestType.TARGET_EXISTS, new TargetDesc("your " + m[3]), undefined, m[2]);
...
function _parse_when(line: string): InterruptCondition {
    ...
    if (m = line.match(/(.*)? (is|be|are) deleted( (in|by) battle)?/i)) {
    ...
    if (m = line.match(/(any of your .*) be played(.*)/i)) {
        logger.warn("only handling evo condition");
        let int_evo: InterruptCondition = {
            ge: GameEvent.PLAY,
            td: new TargetDesc(m[1]),
...
function parse_atomic(line: string, label: string, solid?: SolidEffect2, flags?: any): [AtomicEffect2, string] {
    ...
    if (m = line.match(/^\s*If (.*?),( or (.*),)?(.*)/i)) {
        ...
        iff = m[1]; logger.error("ignoring second clause of IF");
    ...
    */
    ...
    if (m = line.match(/(.*) is also treated as an? (\d+) DP monster( (?:and|that) can.t evolve)?(\s*and gains\s*(＜.*＞))?/i)) {
        //  console.error(2377, m);
