...
import { AtomicEffect, InterruptCondition, StatusCondition, SubEffect, ic_to_plain_text, ic_to_string } from "./effect";
...
function parse_when(line: string): InterruptCondition {
    ...
    if (m = line.match(/(.*)? (is|would be) deleted( (in|by) battle)?/i)) {
...
function parse_atomic(line: string, label: string, solid?: SolidEffect2): [AtomicEffect2, string] {
    ...
    if (m = line.match(/(.*)( may )?evolve(.*) into an? (.*) (in your hand|from your hand|among them) (without paying|for) .?.?.?( evolution)? cost.(.*)/i)) {
        ...
        if (reduce) evocost = parseInt(reduce[3]);
        if (parserdebug) logger.debug(`evo source <${evosource}> evo dest <${evodest}> evo cost <${evocost}>`);
    ...
    if (evodest) {
        ...
        thing.choose = 1;
        thing.td = new TargetDesc(evodest);
        thing.td2 = new TargetDesc(evosource);
        thing.n = 0 + evocost;
        thing.cost_change = [];
        thing.cost_change.push({ n_mod: "reduced", n: evocost });
        //        thing.n_mod = "reduced";
        //      line = "";
    }
    ...
    */
    ...
    if (m = line.match(/trash(ing)? the top card of (your|their) security stack/i)) {
        thing.game_event = GameEvent.CARD_REMOVE_FROM_HEALTH;
        // chosen_target should be a player
        cause: EventCause.EFFECT,
            thing.td = (m[2] == "their") ? new TargetDesc("opponent") : new TargetDesc("me"),
            thing.n = 1;
        line = "";
    }
    ...
    */
    ...
    if (m = line.match(/(.*)\s+(get|gain).?\s*([-0-9+]* DP(.*))/i)) {
        ...
        // this may be a cheat because the next test ought to be two separate clauses
        if (target.toLowerCase().startsWith("all of") && !target.match(/security monster/)) {
            thing.choose = 0;
            target = target.after("all of ");
        }

        //All of your opponent's security monster get -3000 DP."
    ...
    if (m = line.match(/This monster may attack an opponent.s Monster.(\s*With this effect, it can attack the turn it was played)?/i)) {
        ...
        atomic.optional = true;
        thing.choose = 1;
        // We're cheating now, but we should put something in here to indicate the
        // effect can't attack players.
        thing.td = new TargetDesc("your opponent's monster");
        line = "";
    }
