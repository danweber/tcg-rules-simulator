...
export function new_parse_line(line: string, card: (Card | undefined), label: string, inherited: boolean): [Card2, SolidEffect2, AtomicEffect2[], string] {
    ...
    */
    ...
    if (m = line.match(/^(([^.]+?)\.\s)?(([^.]*?).\s*For each([^.]*?)\.)(.*)/)) {
        if (m[5].includes("add")) modify_test = true;
        if (modify_test) {
            look_backwards = m;
        }
    }
...
function parse_if(line: string): GameTest {
    ...
    if (m = line.match(/\[(.*)\] is in this Monster's evolution cards$/i)) {
        logger.error("always true test");
        return new GameTest(GameTestType.TARGET_EXISTS, new TargetDesc("this monster"));
    }
    ...
    if (m = line.match(/the attacking monster is (.*)/i)) {
        ...
        return new GameTest(GameTestType.ATTACKER_IS, target);
...
function _parse_when(line: string): InterruptCondition | InterruptCondition[] {
    ...
    if (m = line.match(/((one|any) of )?(.*) evolve into (a )?(.*)/i)) {
    ...
    if (m = s.match(/^()(get|gain).?\s*(＜.*＞)\s*$/i)) {
        ...
        for (let word of words) {
            if (word.match(/Alliance/i)) {
                ...
                let [_0, nested_solid_effect, _1] = new_parse_line(word, undefined, word, false); // how could we pass through a card here, we won't have any card keywords
...
function parse_atomic(line: string, label: string, solid?: SolidEffect2, flags?: any): [AtomicEffect2, string] {
    ...
    if (m = line.match(/activate the effect below\s*・\s*(.*)/i)) {
        line = m[1];
        logger.info("stripping, down to " + line);
    }
    ...
    if (foreach) {
        ...
        } else if (m = foreach.match(/(?:of )?(.*) (trashed|returned|placed|suspended) by this effect/i)) {
    ...
    */
    ...
    if (m = line.match(/place (this tamer) to (?:1|one) of (.*)/i)) {
    ...
    if (true) {
        ...
        if (line.toLowerCase().startsWith("all of")) {
            ...
            //    console.error(2514, blanket, line, expiration);
    ...
    if (m = line.match(/set (?:it|your memory) to (\d)/)) {
        //        thing.choose = parseInt(m[1]);
