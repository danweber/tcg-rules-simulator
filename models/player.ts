...
import { SolidEffect, StatusCondition, SubEffect } from './effect';
...
export class Player {
    ...
    add_status_cond(sub: SubEffect) { // s: StatusCondition) {
        ...
        if (s.exp_description) {
            ...
            let a: any = null; a.constant_effect_on_player();
    ...
    do_effect(fx: SolidEffect): void {
        ...
        let x = new ResolutionLoop(this.game, [fx]);
    ...
    use(hand_number: number, card: Card) {
        ...
        this.game.pay_memory(card.p_cost);
    ...
    // Returns array of pairs, first is the target inst, second
    // is the cost. 
    // (Cost *could* be more than we have available.)

    // target only checks one thing, unused right now
    // if nothing, return false.
    can_evolve(card: Card, blast: boolean = false, target?: number
    ...
    get_instance(instance_id_or_label: number | string) {
    ...
    execute(words: string[]): void {
        ...
        let arg3 = parseInt(words[3]); // only for digicost right now
        ...
        if (cmd == "PLAY") {
            ...
            let arg1 = this.get_hand_index(word1);
            let card = this.hand[arg1];
            let digimon = this.get_instance(arg2);
            if (!card || !digimon) return;
            msg += `EVOLVE ${card.name.toUpperCase()} ONTO ${digimon.get_name().toUpperCase()}`;
            g.la(msg);
            this.game.no_control();
            this.evolve(arg1, arg2, { cost: arg3 });
            this.game.step();
        } else if (cmd.startsWith("ATT")) {
            let digimon = this.get_instance(word1);
            if (!digimon || (arg2 && !this.get_instance(word2))) return;
            let target = arg2 ? this.get_instance(word2).get_name().toUpperCase() : "PLAYER";
            msg += `ATTACK ${digimon.get_name().toUpperCase()} INTO ${target}`;
            g.la(msg);
            this.game.no_control();
    ...
    // short form is like {"PLAY":[0],"DIGIVOLVE":{},"ATTACK":{},"MAIN":{}}
    ...
    get_x_plays(args: Args): string {
        ...
        for (let i = 0; i < this.hand.length; i++) {
            ...
            let _digi = this.can_evolve(card);
            if (_digi) {
                let digi = _digi.map(x => x[0]);
                can_evolve[i] = digi;
                for (let _target of _digi) {
                    ...
                    v_value = `Digivolve ${str} onto ${this.get_instance(target).get_name()} (${cost})`;
        ...
        for (let i = 0; i < this.field.length; i++) {
            let instance = this.field[i];
            let targets = instance.can_attack();
            if (targets) {
                can_attack[instance.id] = targets;
                for (let target of targets) {
                    v_key = `ATTACK ${this.field[i].id} ${target}`;
                    let s_target = target ? this.get_instance(target).get_name() : "player";
                    v_value = `Attack ${instance.get_name()} into ${s_target}`;
    ...
    evolve(hand_number: number, instance_id: number,
        ...
        if (!instance) {
            logger.warn("digivolving onto " + instance_id + " not found");
            return false;
        }
        let old_tag = `${instance.get_level()}: ${instance.name()}`;
        let can_digi = instance.can_evolve(card, 99);
        if (!can_digi) {
            logger.warn("can't evolve here");
            if (!force) return false;
        }
        if (args && args.cost) {
            if (can_digi && !can_digi.includes(args.cost)) {
    ...
    _set_up(key: string, blob: string, testmode: number): void {
        ...
        if (key.startsWith("DECK")) {
            ...
            pile = this.security; location = Location.EGGDECK;
            ...
            for (let i = 0; i < instances.length; i++) {
                ...
                for (let j = cards.length - 1; j >= 0; j--) {
                    ...
                    if (c) {
                        ...
                        logger.silly("stack is also " + thing.card_ids());
    ...
    attack(source: number, target: number, force = false) {
        ...
        if (!my_targets || !my_targets.includes(target)) {
            logger.error("not legit attcak");
            return;
        }
        ...
        let dsel = new DirectedSubEffectLoop(this.game, e);
    ...
    UNUSED_ui_dump_better(self: boolean, controls: string): string {
        let skeleton: string = "";
    ...
    JSON_player(self: boolean = false) {
        ...
        let moves = self ? JSON.parse(this.get_x_plays({ verbose: true })) : null;
        ...
        if (self == true) {
            hand['cards'] = this.hand.map(x => x.id);
        }
