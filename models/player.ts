...
export class Player {
    ...
    static evo_options_into_questions(evos: Array<[CardLocation, CardLocation | Instance, CardLocation | Instance | undefined, number?]>, blast: boolean = false): UserQuestion[] {
    ...
    get_all_evolves(blast: boolean,
        ...
        mod?: string):
        (Array<[CardLocation, CardLocation | Instance, CardLocation | Instance | undefined, number?]> | false) {
        let ret: Array<[CardLocation, CardLocation | Instance, CardLocation | Instance | undefined, number?]> = [];
        ...
        for (let cl of cards) {
            if (mod?.includes("ignore requirements")) {
                ...
                lefts.forEach(l => ret.push([cl, l, undefined, cost]));
            } else {
    ...
    ): (Array<[CardLocation | Instance, CardLocation | Instance | undefined, number?]> | false) {
        let ret: Array<[CardLocation | Instance, CardLocation | Instance | undefined, number?]> = [];
            ...
            if (lefts && rights) {
                ...
                for (let left of lefts) {
                    for (let right of rights) {
                        ...
                        let e;
                        if (e = card.can_merge_evolve(left, right)) {
                            logger.debug("can merge lr match");
                            // forcing merge cost to be 0, not tecnically true
                            ret.push([left, right, 0]);
                        }
                        if (e = card.can_merge_evolve(right, left)) {
                            logger.debug("can merge rl match");
                            // forcing merge cost to be 0, not tecnically true
                            ret.push([right, left, 0]);
                        }
                ...
                if (!blast || card.can_fusion_blast()) {
                    ...
                    for (let field of this.field) {
                        for (let cl of cards_to_search) {
                            ...
                            if (cost = card.can_merge_evolve(field, cl)) {
                                ret.push([field, cl, cost[0]]);
                                logger.info("CAN FUSE FIELD AND HAND");
                            }
                            if (cost = card.can_merge_evolve(cl, field)) {
                                ret.push([cl, field, cost[0]]);
        ...
        if (fusion != "only") {
            if (!blast || card.can_blast()) {
                ...
                if (egz) {
                    //if (!target || target == egz.id)
                    {
                        ...
                        if (costs) for (let cost of costs) {
                            ret.push([egz, undefined, blast ? undefined : cost]);
                        }
                    }
                }
                let tgt = lefts;
                ...
                for (let mon of tgt) {
                    ...
                    if (costs) for (let cost of costs) {
                        ret.push([mon, undefined, blast ? undefined : cost]);
                    }
                }
            }
        }
    ...
    get_x_plays(args: Args): string {
        ...
        let sample = [
            {
                1: [
                    { "play (3)": "PLAY 3" },
                    { "evo on I7 Sword (2)": "EVO 3 2 0 2" }
                ],
                3: [],
                4: [{ "a": "VOV" }]
            }

            //                "play (3)", "evolve on I3 Dagger (2)" }
        ];
        ...
        if (all_evos) {
            for (let [into, left, right, cost] of all_evos) {
                ...
                let fusion = right ? ` and ${right.get_name()}` : ``;
                let prefusion = right ? `DNA ` : ``;
                let cost_s = (cost == undefined) ? "-" : cost;
    ...
    evolve(hand_number: number, instance_id: number, instance2_id: number | undefined,
