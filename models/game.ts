...
import { Conjunction, SpecialCard, SpecialInstance, TargetDesc, TargetSource, fSpecialPlayer } from './target';
...
export class Game {
    ...
    constructor(master: Mastergame, gid: string, bots: number[], p1string = "", p2string = "", bot1 = "", bot2 = "", seed = "") {
        logger.info(`ctor: ${gid} ${bot1},${bot2},${bots} ${p1string} ${p2string}`);
        this.bots = [];
        this.mode = "fast";
    ...
    public start(): void {
        let p1_seed = this.seed ? this.seed : this.gid;
        let p2_seed = this.seed ? this.seed : this.gid;
        p2_seed += "x";
    ...
    evolve(p: Player, cl: CardLocation, onto: Instance, onto2: Instance | undefined, cost: number|undefined, mode: string) {
        ...
        if (mode === "burst") {
            ...
            e.push({
                ...
                chosen_target: onto2, 
                spec_source: cl, // this "causes" the evolve, but is distinct from what we digi onto
                td: td,
                n_player: p.player_num, // speifically for counter
                label: "bounce"
            });
            let [, pend, ,] = new_parse_line("trash the top card of this monster", undefined, "burst trash", false);
            ...
            e.push({
                ...
                delayed_effect: pend,
                choose: 1,
                delayed_trigger: Phase.END_OF_TURN,
                chosen_target: onto, 
    ...
    find_target(t: TargetDesc, ge: GameEvent, s: TargetSource, sel: SolidEffectLoop | false,
        ...
        if (t.conjunction == Conjunction.LAST_THING) {
            ...
            if (!sel) {
                ...
                while (n > 0) {
                    ...
                    if (eff.chosen_target && eff.chosen_target.kind === "Instance" && eff.chosen_target.top()) {
        ...
        let bb = this.instances.filter(x => x.in_play() && t.matches(x, s, this));
    ...
    preflight_actions(e: SubEffect[]): SolidEffect[] {
        ...
        for (let i = e.length - 1; i >= 0; i--) {
            ...
            if (s.chosen_target && s.chosen_target.location == Location.EGGZONE) {
                ...
                if (s.game_event == GameEvent.PLAY || s.game_event == GameEvent.USE) {
                    let c: Card = s.chosen_target; // the card we're playing
                    let pi = c.play_interrupt;
                    if (pi) {
                        logger.info("INTERRUPTING PLAY/USE " + GameEvent[s.game_event]);
                        let l = c.get_location();
                        let id = c.id;
                        let cl = new CardLocation(this, s.n_player!, l, id);
                        // do we need to mark_outbound_solid_effedts()
                        pi.source = new SpecialCard(cl);
                        pi.trigger_incidents = e;
                        ret.push(pi);
                    }
                }
            }
        ...
        */
        ...
        if (this.phase == Phase.END_OF_TURN) {
            ...
            if (this.memory >= 0) {
                ...
                this.control = this.turn_player;
                this.log("Memory at least 0, turn continues.")
                this.phase = Phase.MAIN;
                this.check_control();
                return true;
            }
