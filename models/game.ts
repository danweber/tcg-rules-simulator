...
import { Conjunction, TargetDesc, TargetSource, fSpecialPlayer } from './target';
...
export class Game {
    ...
    set_last_thing(x: Instance[] | CardLocation[]) {
        console.error(`setting last thing to ${x.map(x => x.get_name())}`);
        this.last_thing = x;
    }
    get_last_thing(): Instance[] | CardLocation[] { return this.last_thing!; }

    constructor(master: Mastergame, gid: string, bots: number[], p1string = "", p2string = "", bot1 = "", bot2 = "") {
        this.bots = [];
        this.mode = "fast";
        ...
        this.Player1 = new Player(p1name, 1, this, master, p1string);
        this.Player2 = new Player(p2name, 2, this, master, p2string);
    }
    ...
    wait(n_player: number, choices: UserQuestion[], question: string = "Choose:", choose: number = 1, mod: string = "") {
        ...
        if (!(n_player in { 1: 1, 2: 2 })) {
            ...
            logger.error("controlx going to " + n_player);
            console.trace();
            this.control = 3
            this.wait_control = 3;
    ...
    get_auto_answers(answer: number): string[] {
        ...
        if (this.wait_question.includes("Choose up to")) {
            // what was this supposed to do? How can we return multiple values at all?
            count = Math.min(this.wait_choices.length, this.wait_count);
        }
    ...
    evolve(p: Player, cl: CardLocation, onto: Instance, cost?: number) {
        ...
        logger.error("should be evolving into a cardlocation, not a card");
        let e: SubEffect[] = [];
    ...
    find_target(t: TargetDesc, ge: GameEvent, s: TargetSource, search_loc: Location = Location.UNKNOWN
        ...
        if (t.conjunction == Conjunction.LAST_THING) {
            if (!this.last_thing) {
                ...
                logger.error("getting last thing!");
        ...
        if ([
            ...
            let ret = to_search.filter(x => t.matches(x, s));
    ...
    process_game_flow(): boolean {
        ...
        if (this.phase == Phase.BEFORE_START_OF_TURN) {
            ...
            if (rules_effects) {
                ...
                // I'm only 
                let int_loop = new InterrupterLoop(this, rules_effects, -5);
