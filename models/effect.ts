...
import { GameEvent, EventCause } from './event';

import { Phase, PhaseTrigger, GameStep } from './phase';
import { SolidEffect2, new_parse_line } from './newparser';
import { CardLocation, DigivolveCondition, KeywordArray } from './card';
import { Interface } from 'readline';
import { InterrupterLoop } from './effectloop';
...
//"maineffect": "If you have a [Pulsuemon] or a Digimon with [Pulsemon] in its evolution cards in play, you may use this card without meeting its color requirements.\r\n\
//[Main] If you have 3 or morkeyworue security cards, 1 of your opponent's Digimon gets -12000 DP for the turn. If you have 3 or fewer security cards, place 1 of your opponent's Digi\
//mon face down on top of their security stack.",
...
export class SolidEffect {
	...
	can_activate?: (e: SolidEffect) => boolean;
	...
	// 
	alternate_allow2: TargetDesc; // this shouldn't be here, it should be on Card
...
//	atomics: AtomicEffect2[];
	...
	constructor(str: string, label: string) { // , parsed: any[]) {
		...
		let [a, b, c] = new_parse_line(str, undefined, label, false);
		console.log("..... " + label);
...
//		this.atomics = c;
		...
		//[All Turns] When an attack target is switched, by suspending this tamer, <Draw 1> and 1 of your Digimon gets +2000 DP for the turn.
		...
		console.log("BEFORE:");
		console.log(this.respond_to);
		console.log("AFTER:");
		console.log(this.solid2.responds_to);
		console.log("DONE");*/
	...
	toString(): string {
		...
		if (this.interrupt) ret += "RESPOND: " + this.interrupt.toString();
...
// "1 of your digimon with [Greymon] in its name"
// from "Instance 6"
...
//	digicond?: DigivolveCondition;
	// DELETE "immunity"
	//	immunity?: SubEffect;  // do I need a whole struct? what
	// if this were just a boolean, meaning 
}



// if your opponent has (a digimon) in play
// if you have (a red tamer) in play
// while you have (a tamer with TRAIT) in play
// while this digimon (has [greymon] in name)
// while this digimon (is suspended)
// if you have (a digimon that's black or has TRAIT) in play,
...
// has "cannot have DP changed by opponent's digimon effects"
/*
let oppo_digi: TargetDesc = _blank;
oppo_digi.conjunction = Conjunction.ALL;
targets = [new SubTargetDesc("other"), new SubTargetDesc("digimon")];
oppo_digi.targets = targets;
let immune_to_DP_change_by_opponent_digimon: StatusCondition = {
	s: {
...
let sub_eff: SubEffect = {
	...
	td: tgt,
	status_condition: immune_to_DP_change_by_opponent_digimon
};
*/
// has "is immune to opponent's digimon's effects forever"

/*let immune_to_oppo_digi_effects: StatusCondition = {
	s: {
...
// cannot gain memory by digimon effects
...
// 6. You can DIGIVOLVE by rules.
//... Okay, I guess I should have flags to
//    distinguish "effect" v "rules" v "battle"
// cannot have DP changed by opponent's digimon's (effect)
...
export function subeffect_to_string(sub: SubEffect): string {
	let ret = "";
	if (sub.immune) { ret += "CAN'T"; }
	ret += GameEvent[sub.game_event].replaceAll("_", " ");
	//	ret += show number / string / whatever.
	return ret;

}

export function status_cond_to_string(sc: StatusCondition) {
