...
export class SolidEffect {
	rules?: boolean; // fake solid event caused by game rules
...
export function Solid_is_triggered(solid?: SolidEffect) {
	...
	return (solid.interrupt || solid?.keywords.includes("[Main]")  || solid.phase_trigger || solid.respond_to.length > 0);
...
export function ic_to_plain_text(ic: InterruptCondition): string {
	...
	if (ic.ge == GameEvent.CARD_REMOVE_FROM_HEALTH) {
		return `remove a card from health`; // which health?
	}
	...
	if (ic.cause) msg += `Cost: ${EventCause[ic.cause]}`;
	if (ic.source) msg += ` from ${ic.source.toString()}`;
	return msg;
}
