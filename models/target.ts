...
export class GameTest {
    ...
    toPlainText(): string { return this.td.toPlainText(); }
...
export class TargetDesc {
    ...
    toString(): string {
        ...
        if (this.conjunction != Conjunction.ALL) {
            logger.error("OH NO 2 " + this.conjunction);
            return "ERROR";
        }
    ...
    matches(t: Instance | CardLocation, s: TargetSource): boolean {
        ...
        if (this.conjunction == Conjunction.SELF) {
            ...
            logger.debug(`insatnce ${s.is_instance()} and card ${s.is_card()} extract ${("extract" in t) ? "TRUE" : "FALSE"}`);
            // to see if an *instance as source* self-matches
            // to a cardLocation target,
            logger.debug(`tid is ${t.id} and sid is ${s.id()}`);
            //return (t.id == s.id());
        ...
        logger.debug("AND case returned " + answers.join(",") +
            " " + this.targets.map(x => x.toString()).join(","));
    ...
    constructor(_text: number | string, type: string = "instance") {
        ...
        if (text == "self" || text == "this monster") {
        ...
        if (text == "it" || text.match(/that monster/)) {
            //        logger.warn("assuming it means self");
        ...
        if (m = text.match(/^\[(.*)\]$/)) {
            this.conjunction = Conjunction.UNIT;
            this.targets.push(new SubTargetDesc("name", m[1]));
            text = "";
        }
        ...
        */
        ...
        if (m = desc2.match(/with the (lowest|highest) (dp|level|play cost)/i)) {
            ...
            switch (m[2].toLocaleLowerCase()) {
                case "play cost": this.most_kind = StatusTestWord.PLAY_COST; break;
...
export class SubTargetDesc {
    ...
    static full_candidate(str: string): (TargetDesc | SubTargetDesc)[] | false {
        ...
        logger.info("trying full " + str);
        let std_candidate;
        while (std_candidate = SubTargetDesc.candidate(str)) {
            targets.push(std_candidate[0]);
            str = std_candidate[1];
            logger.info(`trying pushing ${std_candidate[0]} remaining ${str}`);
        }
        str = str.trim();
        logger.info("done, str is " + str);
        logger.info("done, tgt is " + targets.toString());
        if (str.length < 2) { return targets; }
    ...
    static _candidate(str: string): [TargetDesc, string] | [SubTargetDesc, string] | false {
        ...
        if (m = str.match(/^from (your )?(hand or trash|trash|security( stack))(.*)/i)) {
            let x = new SubTargetDesc(m[2]);
            return [x, m[4].trim()]
        }

        if (str.startsWith("1 card")) {
            str = str.after("1 card").trim()
        }
        ...
        if (m = str.match(/^your opponent'?'? (.*)$/i)) {
        ...
        if (m = str.match(/(.*)\s*level (\d+)\s*(or (higher|lower))?\s*(.*)$/)) {
            let prefix = m[1];
            let level = m[2];
            let compare = m[4]
            let suffix = m[5];
        ...
        if (m = str.match(/^(with )?(\d+)\s*DP (or (more|less))?.?.?\s*/i)) {
            //        logger.silly("PLAYCOST MATCH");
            let dp = m[2];
            let x = new SubTargetDesc("DP", m[4], dp);
            return [x, ""]
        }

        if (m = str.match(/^\s*(with )?evolution cards\s*$/i)) {
            //			logger.silly("2DESC2 is " + desc2); // with x in its name
            //			logger.silly(`m[3] is ${m[3]} x`);
            let x = new SubTargetDesc("sourcecount", "more", "1");
            return [x, ""]
        }

        if (m = str.match(/^(with )?(\[.*\])\sin.(its|their).name\s*$/)) {
            ...
            let x = new SubTargetDesc("name_contains", m[2].trim());
            return [x, ""]
        }
