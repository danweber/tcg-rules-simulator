        ...
        if (this.conjunction == Conjunction.SELF || this.conjunction == Conjunction.ANOTHER) {
            ...
            if (s.is_instance() && ("extract" in t)) {
                ...
                if (t.location == Location.FIELD) {
                    ...
                    return (t.location == s.location() && s.id() == t.instance) === self;
                }

                // source is an instant, looking for SELF CARD
                return (t.location == s.location() &&
                    t.card_id == s.card_id()
                    && t.card == s.get_instance().top()) === self;
            ...
            if (s.is_card()) {
                ...
                return (t.location == s.location() &&
                    t.card_id == s.card_id()) === self;

            }
            return (t.id == s.id()) === self;
        }
        if (this.conjunction == Conjunction.SOURCE) {
...
*/
        ...
        //                  1       2       3                        4   5  
        if (m = str.match(/^(with )?(\d+)\s*(DP|play.?cost|use cost) (or (more|less))?.?.?\s*/i)) {
            //        logger.silly("PLAYCOST MATCH");
