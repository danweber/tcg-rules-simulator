...
export class TargetDesc {
    ...
    toPlainText(): string {
        ...
        return this.targets.map(x => x.toPlainText()).join(" ");
    ...
    constructor(_text: number | string, type: string = "instance") {
        ...
        logger.debug(`TARGET CTOR:<${_text}>`);
        this.targets = [];
        ...
        */
        ...
        // move each clause we can into SubTargetDesc
        std_candidate = SubTargetDesc.candidate(desc1);
        if (std_candidate) {
            this.targets.push(std_candidate[0]);
            desc1 = std_candidate[1];
        }


        if (m = desc2.match(/(.*) (and|with) (.*)/)) {
            logger.debug(m[1]);
            logger.debug(m[3]);
            let s1 = SubTargetDesc.candidate(m[1]);
            let s2 = SubTargetDesc.candidate(m[3]);
            // if both match, then go for it
            // overwrite existing calsues?
            if (s1 && s2) {
                logger.info("and hit");
                this.targets.push(s1[0], s2[0]); // AND                 
                return;
            }
        }
        ...
        if (m = desc2.match(/(.*) or (.*)/)) {
            ...
            logger.debug(m[2]);
            let s1 = SubTargetDesc.candidate(m[1]);
            let s2 = SubTargetDesc.candidate(m[2]);
            if (s1 && s2) {
                logger.info("or hit");
                let or = new TargetDesc("");
                or.conjunction = Conjunction.OR;
                or.targets = [s1[0], s2[0]];
                return or;
            }
        }
...
export class SubTargetDesc {
    ...
    static candidate(str: string): [TargetDesc, string] | [SubTargetDesc, string] | false {
        ...
        if (m = str.match(/^other (.*)$/i)) {
            return [ new TargetDesc("not self"), m[1] ]
        }
        ...
        if (m = str.match(/^(with )?a play.?cost of (\d+)\s+(or (more|less))?\s*/i)) {
            //        logger.silly("PLAYCOST MATCH");
            let playcost = m[2];
            let x = new SubTargetDesc("playcost", m[4], playcost);
            return [x, ""]
        }
    ...
    do_compare(a: number, s: TargetSource) {
        ...
        if (relative == "targetsource-dp") {
            let i = s.get_instance();
            c = i.dp();
            console.info("relative set c to " + c);
        }
        console.debug(`a ${a} b ${COMPARE[b]} c ${c}`);
